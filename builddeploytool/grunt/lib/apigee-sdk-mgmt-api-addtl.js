...
/** 
	 Due to inconsistencies on how apigee-profile is handled in the SDK
     for KVM's, use this code instead of the common library for now
**/
...
exports.importSharedFlowBundle = function(apigee_profiles, callback, includeCurl){
	var apigeel = apigee_profiles[apigee_profiles.env];
	var fs = require('fs');
	var rs = fs.createReadStream('target/' + apigeel.sharedflowbundle + '.zip');
	var options = {
		method: 'POST',
		uri: apigeel.url_mgmt + '/v1/organizations/' + apigeel.org + '/sharedflows?action=import&name=' + apigeel.sharedflowbundle,
	 	//uri: apigeel.url_mgmt + '/v1/o/' + apigeel.org + '/sharedflows/' + apigeel.sharedflowbundle + '/revisions',
		auth: {
			user: apigeel.username,
			password: apigeel.password
		},
...
exports.getDeployedSharedFlowRevisions = function (apigee_profiles, callback, includeCurl) {
	...
	var options = {
		method: 'GET',
		uri: apigeel.url_mgmt + '/v1/o/' + apigeel.org + '/environments/' + apigeel.env + '/sharedflows/' + apigeel.sharedflowbundle + '/deployments',
		auth: {
			user: apigeel.username,
			password: apigeel.password
		}
...
exports.undeploySharedFlowRevision = function(apigee_profiles, revision, callback, includeCurl){
	//echo curl -H "Authorization:Basic $credentials" "$url/v1/organizations/$org/apis/$application/revisions/$RevToUndeploy/deployments?action=undeploy&env=$environment" -X POST -H "Content-Type: application/octet-stream"
	var apigeel = apigee_profiles[apigee_profiles.env];
	var options = {
		method: 'POST',
		uri: apigeel.url_mgmt + '/v1/o/' + apigeel.org + '/sharedflows/' + apigeel.sharedflowbundle + '/revisions/' + revision + "/deployments?action=undeploy&env=" + apigeel.env,
		auth: {
			user: apigeel.username,
			password: apigeel.password
		},
...
exports.deploySharedFlowRevision = function(apigee_profiles, revision, callback, includeCurl){
	//echo curl -H "Authorization:Basic $credentials" "$url/v1/organizations/$org/apis/$application/revisions/$RevToDeploy/deployments?action=deploy&env=$environment" -X POST -H "Content-Type: application/octet-stream"
	//curl -X POST -H 'Content-type:application/x-www-form-urlencoded' https://api.enterprise.apigee.com/v1/o/testmyapi/environments/test/apis/forecastweather-grunt-plugin-api/revisions/86/deployments\?override\=true\&delay\=10
	var apigeel = apigee_profiles[apigee_profiles.env];
	var options = {
		method: 'POST',
		uri: apigeel.url_mgmt + '/v1/o/' + apigeel.org + '/e/' + apigeel.env + '/sharedflows/' + apigeel.sharedflowbundle + '/revisions/' + revision + '/deployments' + (apigeel.override ? '?override=true' : '') + ((apigeel.override && apigeel.delay != 0) ? '&delay=' + apigeel.delay : ''),
		auth: {
			user: apigeel.username,
			password: apigeel.password
		},
